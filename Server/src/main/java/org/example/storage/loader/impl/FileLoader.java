...
import org.example.storage.loader.xml.WorkerHandler;
import org.example.storage.loader.xml.XmlWorkerWriter;
...
public class FileLoader implements ILoader<Collection<Worker>> {
    private static final String XML_FILE_PATH = System.getenv("WORKER_XML_FILE_PATH");
    ...
    public boolean save(Collection<Worker> workers) {
        ...
        try (var out = new BufferedOutputStream(new FileOutputStream(XML_FILE_PATH))) {
    ...
    public Collection<Worker> load() {
        ...
        try (var isr = new InputStreamReader(new FileInputStream(new File(XML_FILE_PATH)))){
